...
st.set_page_config(page_title="만능 올인원 RAG 챗봇", page_icon="🤖", layout="wide")
...
# ------------------------------------------------------------------
# 2. 메인 화면 컨트롤 영역 (대화 기억 토글 + 텍스트 1개 + 엑셀/CSV 2개)
# ------------------------------------------------------------------
st.header("⚙️ 챗봇 설정 컨트롤러")
remember_chat = st.toggle(
    "현재 대화 기록 기억하기",
    value=True,
    help="이 기능을 켜면 대화 내용이 노트북에 영구 저장되어 다음에 앱을 켜도 이어집니다.",
)

with st.expander("📂 데이터 지식베이스 구축", expanded=True):
...
if "vector_store" not in st.session_state:
    if os.path.exists(INDEX_DIR):
        ...
        st.info("💾 기존에 저장된 데이터 지식베이스를 불러왔습니다!")
    else:
...
if train_button:
    ...
    if text_file is not None:
        ...
        st.error("⚠️ 최소한 '기본 참고 문서(.txt)' 파일은 업로드해야 학습이 가능합니다.")

# ------------------------------------------------------------------
# 5. 왼쪽 사이드바 대화 화면 렌더링 및 입력 처리 (구글 실시간 검색 연동 포함)
# ------------------------------------------------------------------
with st.sidebar:
    ...
    user_query = st.chat_input("문서와 데이터에 대해 질문하세요...")
            ...
            with st.spinner("생각 중..."):
                try:
                    ...
                    if st.session_state.vector_store is not None:
                        ...
                        answer = "메인 화면에서 파일을 업로드하고 [지식베이스 구축] 버튼을 누르면 파일 기반 분석 대화가 가능합니다!"
